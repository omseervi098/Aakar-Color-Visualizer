...
const ColorVisualiser = () => {
  ...
  const coloringWall=(coords)=>{
    let temp="[";
    for(let i=0;i<coords.length;i+=2){
      temp+=`{'x':${coords[i]},'y':${coords[i+1]}},`;
    }
    // erase last comma
    temp=temp.slice(0,-1);
    temp+="]";
    ...
    formData.append("newCl",'0,255,0');
    formData.append("segment","WALL" );
    axios({
      method:"GET",
      url:"http://54.85.47.175/color",
      data:formData,
    })
    .then((res)=>{
      console.log(res);
        ...
        <div className="colorvisualiser__container container-fluid">
          <div className="row ">
            <div className="col-12 col-md-10 colorvisualiser__container__left">
              <DisplayImg result={result} file={file}  coloringWall={coloringWall} fresult={fresult} />
            </div>
